...
class AIService {
     ...
     */
    runFallbackParser(userQuery, session) {
        ...
        if (/show\s+([\w\s]+)\s+cases\s+in\s+([\w\s]+)/i.test(query)) {
            ...
            let crime_type = '';
            if (typeStr.includes('theft'))
                crime_type = 'Theft';
            else if (typeStr.includes('cyber'))
                crime_type = 'Cyber Fraud';
            else if (typeStr.includes('burglary'))
                crime_type = 'Burglary';
            else if (typeStr.includes('assault'))
                crime_type = 'Assault';
            else if (typeStr.includes('robbery'))
                crime_type = 'Robbery';
            // District mapping
            let location = locStr;
            if (locStr.toLowerCase() === 'bengaluru')
                location = 'Bengaluru East'; // Default region mapper
            return {
    ...
    async processQuery(userQuery, session) {
        ...
        try {
            ...
            switch (intent) {
                case 'get_fir': {
                    ...
                    const result = await query_service_1.queryService.getFir(firId);
                    finalMarkdown = formatter_service_1.formatterService.formatFir(result.data);
                ...
                case 'criminal_history': {
                    ...
                    const result = await query_service_1.queryService.getAccused(personName);
                    finalMarkdown = formatter_service_1.formatterService.formatAccused(result.data);
                    sqlExecuted = result.sql;
                ...
                case 'get_victim': {
                    ...
                    const result = await query_service_1.queryService.getVictim(personName);
                    finalMarkdown = formatter_service_1.formatterService.formatVictim(result.data);
                    sqlExecuted = result.sql;
                ...
                case 'crime_by_location': {
                    ...
                    const result = await query_service_1.queryService.crimeByLocation(location, crimeType);
                    finalMarkdown = formatter_service_1.formatterService.formatCrimeByLocation(result.data, location, crimeType);
                    sqlExecuted = result.sql;
                    sqlResults = result.data;
                    activeFilters = { location };
                    if (crimeType)
                        activeFilters.crime_type = crimeType;
                    break;
                }
                case 'investigation_status': {
                    ...
                    let status = 'Pending';
                    if (rawStatus.toLowerCase().includes('investig'))
                        status = 'Under Investigation';
                    else if (rawStatus.toLowerCase().includes('compl'))
                        status = 'Completed';
                    else if (rawStatus.toLowerCase().includes('clos'))
                        status = 'Closed';
                    const location = entities.location || session.activeFilters?.location;
                    const crimeType = entities.crime_type || session.activeFilters?.crime_type;
                    const result = await query_service_1.queryService.investigationStatus(status, location, crimeType);
                    finalMarkdown = formatter_service_1.formatterService.formatInvestigationStatus(result.data, status, location, crimeType);
                    sqlExecuted = result.sql;
                    sqlResults = result.data;
                ...
                case 'repeat_offenders': {
                    const location = entities.location || session.activeFilters?.location;
                    const crimeType = entities.crime_type || session.activeFilters?.crime_type;
                    const result = await query_service_1.queryService.repeatOffenders(location, crimeType);
                    finalMarkdown = formatter_service_1.formatterService.formatRepeatOffenders(result.data, location, crimeType);
                    sqlExecuted = result.sql;
                    sqlResults = result.data;
                    if (location)
                        activeFilters.location = location;
                ...
                case 'crime_statistics': {
                    const location = entities.location || session.activeFilters?.location;
                    const crimeType = entities.crime_type || session.activeFilters?.crime_type;
                    const result = await query_service_1.queryService.crimeStatistics(location, crimeType);
                    finalMarkdown = formatter_service_1.formatterService.formatStatistics(result.data, location);
                    sqlExecuted = result.sql;
                    sqlResults = [result.data];
                    if (location)
                        activeFilters.location = location;
