...
function generateFIRs(count, locationCount) {
    const list = [];
...
function generateCrimeLinks(firs, accusedCount, victimCount) {
    const links = [];
    const seen = new Set();
    firs.forEach(fir => {
        ...
        while (localAccused.length < aCount) {
            ...
            if (!localAccused.includes(aId))
                localAccused.push(aId);
        }
        const localVictims = [];
        while (localVictims.length < vCount) {
            const vId = randomRange(1, victimCount);
            if (!localVictims.includes(vId))
                localVictims.push(vId);
        }
        // Associate
...
function generateAllData() {
    ...
    const sqlLines = ['-- PostgreSQL Seeding Script', 'BEGIN;'];
    // Locations insert
    ...
    const sqlPath = path_1.default.resolve(__dirname, '../../db/postgres/seed.sql');
    // Ensure directory exists
    if (!fs_1.default.existsSync(path_1.default.dirname(sqlPath))) {
        fs_1.default.mkdirSync(path_1.default.dirname(sqlPath), { recursive: true });
    }
    fs_1.default.writeFileSync(sqlPath, sqlLines.join('\n'), 'utf8');
