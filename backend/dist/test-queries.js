...
require("./db/sqlite"); // Boots SQLite and seeds database
...
async function runTests() {
    ...
    const session = context_service_1.contextService.getOrCreateSession(sessionId);
    for (const query of TEST_CASES) {
        console.log(`\n\n🔎 INVESTIGATOR QUERY: "${query}"`);
        console.log('-----------------------------------------------------------');
        try {
            // 1. Language detection and translation to English if Kannada
            const detectedLang = await translation_service_1.translationService.detectLanguage(query);
            let processedMessage = query;
            if (detectedLang === 'kn') {
                processedMessage = await translation_service_1.translationService.translate(query, 'en');
                console.log(`[Translation] Detected Kannada. Translated: "${processedMessage}"`);
            }
            // 2. Process query
            const result = await ai_service_1.aiService.processQuery(processedMessage, session);
            // 3. Translate response back if needed
            let finalResponseText = result.text;
            if (detectedLang === 'kn') {
                finalResponseText = await translation_service_1.translationService.translate(result.text, 'kn');
            }
            ...
            context_service_1.contextService.addMessage(sessionId, 'user', query);
            context_service_1.contextService.addMessage(sessionId, 'model', finalResponseText, result.sql, result.sqlResults, {
    ...
    const ctxSession = context_service_1.contextService.getOrCreateSession(ctxSessionId);
    // 1. Ask about Cyber Fraud in Bengaluru
    const q1 = 'Show cyber fraud cases in Bengaluru';
    console.log(`\nInvestigator Query 1: "${q1}"`);
    const r1 = await ai_service_1.aiService.processQuery(q1, ctxSession);
    console.log(`Active Location Filter: ${r1.activeFilters?.location}`);
    console.log(`Active Crime Filter:    ${r1.activeFilters?.crime_type}`);
    context_service_1.contextService.addMessage(ctxSessionId, 'user', q1);
    context_service_1.contextService.addMessage(ctxSessionId, 'model', r1.text, r1.sql, r1.sqlResults, {
    ...
    const r2 = await ai_service_1.aiService.processQuery(q2, ctxSession);
