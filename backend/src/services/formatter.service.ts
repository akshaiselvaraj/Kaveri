export class FormatterService {
   ...
   * Format FIR Details
   */
  public formatFir(data: any): string {
    if (!data) return '⚠️ **Investigation dossier not found for the requested FIR.**';
   ...
   */
  public formatAccused(data: any): string {
    if (!data) return '⚠️ **Suspect record not found.**';
   ...
   */
  public formatVictim(data: any): string {
    if (!data) return '⚠️ **Victim profile not found.**';
   ...
   */
  public formatCrimeByLocation(results: any[], location: string, crimeType?: string): string {
    let md = `### 📍 Crime Registry for Location: **${location}**\n`;
    if (crimeType) md += `*Filtered by Crime Type: \`${crimeType}\`*\n\n`;
   ...
   */
  public formatInvestigationStatus(results: any[], status: string, district?: string, crimeType?: string): string {
    let md = `### 🔍 Investigation Status: **${status}**\n`;
    const filters = [];
    if (district) filters.push(`District: \`${district}\``);
    if (crimeType) filters.push(`Crime Type: \`${crimeType}\``);
    if (filters.length > 0) md += `*Filters: ${filters.join(' | ')}*\n\n`;
   ...
   */
  public formatRepeatOffenders(results: any[], district?: string, crimeType?: string): string {
    ...
    if (results.length > 0) {
      ...
      results.forEach((r: any) => {
        ...
        const crimes = r.crime_types ? r.crime_types.split(',').map((c: string) => `\`${c}\``).join(', ') : 'N/A';
   ...
   */
  public formatStatistics(stats: any, district?: string): string {
    let md = `### 📊 Crime Intelligence Summary Report\n`;
    if (district) md += `*Location Boundary: \`${district}\`*\n\n`;
    ...
    if (stats.crimeTypes.length > 0) {
      ...
      stats.crimeTypes.forEach((t: any) => {
    ...
    if (stats.highRiskSuspects.length > 0) {
      stats.highRiskSuspects.forEach((a: any) => {
